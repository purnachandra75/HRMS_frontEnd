...
function ExperienceLetter({ userName, onLogout }) {
  ...
  useEffect(() => {
    ...
    const loadTemplates = async () => {
      ...
      try {
        const data = await getLetterTemplates('EXPERIENCE');
        if (cancelled) return;
        setTemplates(data);
        const preferred = data.find((t) => t.isDefault) || data[0] || null;
        setSelectedTemplateId(preferred ? preferred.id : '');
      } catch (err) {
        console.error('Failed to load experience letter templates:', err);
      } finally {
  ...
  const generatePDF = async () => {
    ...
    if (hasCustomBody) {
      try {
        await generatePagedPdf(customBodyRef.current, `ExperienceLetter_${employee.employeeId}.pdf`);
      ...
      <section className="rounded-xl border border-border/80 bg-card p-5 shadow-sm">
        ...
        {!templatesLoading && templates.length === 0 ? (
          <div className="rounded-lg border border-[#fecaca] bg-[#fef2f2] px-3 py-2 text-sm text-[#b91c1c]">
            No Experience Letter templates yet.{' '}
